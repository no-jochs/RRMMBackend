...
export default function SearchControls() {
    const [checkboxState, setCheckboxState] = useState([
        { label: 'In-Person', checked: true, icon: <GroupIcon className="cbg-btn-label-icon" sx={{ color: grey, fontSize: "small" }} /> },
        { label: 'Online', checked: true, icon: <DevicesIcon className="cbg-btn-label-icon" sx={{ color: grey, fontSize: "small" }} /> },
        { label: 'Hybrid', checked: true, icon: <CoPresentIcon className="cbg-btn-label-icon" sx={{ color: grey, fontSize: "small" }} /> },
    ]);
    ...
    const toggleChecked = (v) => {
        ...
        for (let i = 0; i < checkboxState.length; i++) {
            if (checkboxState[i].label === v) {
                newCheckboxState.push(Object.assign({ checked: !checkboxState[i].checked }, checkboxState[i]));
            } else {
    ...
    return (
        <Container className="search-controls">
            <Row className="align-items-center">
                ...
                <Col>
                    <CheckboxButtonGroup checkboxDetails={checkboxState} toggleChecked={toggleChecked} />
                </Col>
                <Col>
                    <DropdownButton>
